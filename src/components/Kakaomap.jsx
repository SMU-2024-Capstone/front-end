...
const CustomSelect = () => {
  ...
  const [isSeoulVisible, setIsSeoulVisible] = useState(false);
...
const handleSidoClick = (sido) => {
  ...
  setIsSidoBoxOpen(false);
  setDownIconSrc(down_no);

  setIsGuGunBoxOpen(true);
  setDownIconSrc2(down);

  if (sido === "서울") setIsSeoulVisible(true);
  else setIsSeoulVisible(false);
  if (sido === "부산") setIsBusanVisible(true);
  else setIsBusanVisible(false);
  // 나머지도 동일하게 설정
};
